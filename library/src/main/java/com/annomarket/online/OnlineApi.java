...
public class OnlineApi {
	...
	/**
	 * Construct an <code>OnlineApi</code> accessing a specific processing pipeline
	 * on the AnnoMarket.com platform using the given credentials.
	 * @param item the {@link Item} returned from the {@link Shop} 
	 * which represents the processing pipeline which will be used
	...
	public AnnotatedDocument annotateFileContents(File documentContent, 
			...
			SupportedMimeType documentMimeType) throws IOException, OnlineApiException {
	...
	/**
	 * Annotates a single document publicly available under a given URL. Returns an object 
	 * which allows for convenient access to the annotations in the annotated document
	 * @param documentUrl the publicly accessible URL from where the document will be downloaded
	 * @param documentMimeType the MIME type of the document which will be annotated
	 * @return an {@link AnnotatedDocument} which allows for convenient programmatic access
	 * to the annotated document
	 * @throws OnlineApiException 
	 */
	...
	/**
	 * Annotates a single document and returns an {@link InputStream} from which the 
	 * contents of the serialized annotated document can be read
	...
	public InputStream annotateDocumentAsStream(String documentText, 
			...
			ResponseFormat serializationFormat) throws OnlineApiException {
		...
		try {
			...
			throw new OnlineApiException(msg == null?e.getMessage():msg.asText(), e);
		}
	}
	
	/**
	 * Annotates the contents of a single file returning an {@link InputStream} 
	 * from which the annotated content can be read
	...
	public InputStream annotateFileContentsAsStream(File documentContent, 
			Charset documentEncoding, 
			SupportedMimeType documentMimeType, 
			ResponseFormat serializationFormat) throws IOException, OnlineApiException {
	...
	/**
	 * Annotates a single document publicly available under a given URL. Returns the annotated
	 * document serialized into the specified format
	 * @param documentUrl the publicly accessible URL from where the document will be downloaded
	 * @param documentMimeType the MIME type of the document which will be annotated
	 ...
	 * This is done by constructing the appropriate OnlineServiceRequest object. Returns the contents of the annotated document
	 ...
	 * @return an{@link InputStream} for the serilization of the annotated document in the specified format
	 * @throws OnlineApiException 
	 */
		...
		try {
			if(requestCompression) {
				return client.requestForStream("", "POST", rq, 
						"Accept", serializationFormat.acceptHeader, "Accept-Encoding", "gzip");
			} else {
				return client.requestForStream("", "POST", rq, 
						"Accept", serializationFormat.acceptHeader);
			}
		} catch(RestClientException e) {
			JsonNode response = e.getResponse();
			if(response == null) {
